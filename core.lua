-- CORE.LUA - Main Logic Functions (FIXED PLAYBACK SPEED)
local Core = {}
    ...
    while Config.isPlaying do
        ...
        for i = Config.SavedCP, #Config.TASDataCache do
            ...
            for f = Config.SavedFrame, #data do
                ...
                -- Terapkan Velocity (tanpa SpeedMultiplier, karena speed diatur lewat timing)
                if frame.VEL then
                ...
                if PlaybackSpeed >= 1 then
                    -- Speed >= 1x: Skip frames untuk lebih cepat
                    if f % math.floor(PlaybackSpeed) == 0 then
                        RunService.Heartbeat:Wait()
                    end
                else
                    -- Speed < 1x: Tambah delay untuk lebih lambat
                    local t = tick()
        ...
        -- Loop Logic dengan Auto Respawn
        if Config.isPlaying then
            if Config.isLooping then
                -- Jika Auto Respawn Loop aktif, respawn character
                if Config.autoRespawnLoop then
                    ...
                    if success then
                        ...
                        -- Wait a bit sebelum mulai loop lagi (untuk grab coil, dll)
                        task.wait(1)
                    end
                else
                    -- Loop tanpa respawn
                ...
                -- Reset ke checkpoint awal
                Config.SavedCP = 0
                Config.SavedFrame = 1
            else
                -- Loop disabled, stop playback
